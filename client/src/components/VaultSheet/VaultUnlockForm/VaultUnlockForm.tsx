...
const VaultUnlockForm: React.FC<VaultUnlockFormProps> = ({
  onSuccess,
  onChatClick,
  isSubmitting,
  setIsSubmitting
  ...
  const correctPassword = 'TESOURO123';
  ...
  const handleSubmit = async (e: React.FormEvent | React.KeyboardEvent) => {
    ...
    if (password.toUpperCase() === correctPassword) {
      ...
      // Ativa animação de vibração no botão
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        <div className="relative">
          <input
            ...
            } rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 disabled:opacity-50 disabled:cursor-not-allowed text-center text-xl font-mono tracking-widest transition-all duration-300`}
            onKeyDown={(e) => {
        ...
        <button
          ...
          } disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-bold rounded-2xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-violet-500/50 flex items-center justify-center gap-3 text-lg shadow-lg transform hover:scale-105 active:scale-95`}
        >
          {isSubmitting ? (
            <>
      ...
      <div className="">
        <button 
          ...
          className={`w-full p-4 bg-violet-500/10 hover:bg-violet-500/20 active:bg-violet-500/30 border border-violet-500/30 hover:border-violet-500/50 active:border-violet-500/70 rounded-2xl transition-all duration-200 group transform hover:scale-[1.02] active:scale-[0.98] hover:shadow-lg hover:shadow-violet-500/20 active:shadow-xl ${
