...
interface VaultInteriorGridProps {
  prizes: Prize[];
  onClaimPrize?: (prizeId: string) => void;
  claimingPrize?: string | null;
  claimProgress?: number;
  claimedPrizes?: Set<string>;
  finalProgress?: Map<string, number>;
}

const VaultInteriorGrid: React.FC<VaultInteriorGridProps> = ({
  prizes,
  onClaimPrize,
  claimingPrize,
  claimProgress,
  ...
  const [holdingPrize, setHoldingPrize] = useState<string | null>(null);
  ...
  const handleMouseDown = (e: React.MouseEvent, prizeId: string) => {
    if (claimedPrizes.has(prizeId) || claimingPrize === prizeId) return;
    
    startPositionRef.current = { x: e.clientX, y: e.clientY };
    hasMoved.current = false;
    setHoldingPrize(prizeId);
    
    holdTimerRef.current = setTimeout(() => {
      if (!hasMoved.current && onClaimPrize) {
        onClaimPrize(prizeId);
      }
    }, 1000);
  };

  const handleMouseMove = (e: React.MouseEvent) => {
    if (startPositionRef.current && holdingPrize) {
  ...
  const handleMouseUp = () => {
    ...
    setHoldingPrize(null);
    startPositionRef.current = null;
    hasMoved.current = false;
  };

  const handleTouchStart = (e: React.TouchEvent, prizeId: string) => {
    if (claimedPrizes.has(prizeId) || claimingPrize === prizeId) return;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
      {prizes.map((prize) => {
        const ItemIcon = typeConfig[prize.type].icon;
        const rarity = rarityConfig[prize.rarity];
        const isClaimed = claimedPrizes.has(prize.id);
        const isClaiming = claimingPrize === prize.id;
        const isHolding = holdingPrize === prize.id;
        const progress = finalProgress.get(prize.id) || (isClaiming ? claimProgress : 0);
        ...
        return (
          <div
            ...
            onMouseDown={(e) => handleMouseDown(e, prize.id)}
            onMouseMove={handleMouseMove}
            onMouseUp={handleMouseUp}
            onMouseLeave={handleMouseUp}
            onTouchStart={(e) => handleTouchStart(e, prize.id)}
          ...
          >
            ...
            {/* Item Icon */}
            <div className={`w-16 h-16 rounded-xl ${typeConfig[prize.type].bg} flex items-center justify-center mb-4 mx-auto group-hover:scale-110 transition-transform`}>
              <ItemIcon className={`h-8 w-8 ${typeConfig[prize.type].color}`} />
            </div>
            ...
            <div className="text-center">
              ...
              {prize.quantity && (
                <div className="text-xs text-slate-500 mb-2">
                  Quantidade: {prize.quantity}
                </div>
              )}
              
              {prize.claimDeadline && !isClaimed && (
                <div className="text-xs text-orange-400">
                  Válido por {prize.claimDeadline}
                </div>
              )}
            </div>
...
export default VaultInteriorGrid;